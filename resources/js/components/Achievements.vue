<template>
    <div class="relative flex items-center">
        <div class="w-full overflow-hidden relative h-72">
            <div
                v-for="(slide, index) in arr"
                class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4 absolute inset-0 transform slide transition-all ease-in-out duration-1000"
                :class="index === 0 ? 'translate-x-0' : 'translate-x-full'"
            >
                <div v-for="item in slide">
                    <img :src="item.img" class="w-48 mx-auto">
                </div>
            </div>
        </div>
        <div
