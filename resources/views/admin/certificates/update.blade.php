...
@section('content')
    <div class="max-w-4xl p-4 mx-auto border shadow rounded text-sm">
        <h4 class="text-lg mb-8">Редактировать</h4>

        <form method="POST" action="/admin/certificates/{{$certificate->id}}" enctype="multipart/form-data">
            @csrf
            @method('PATCH')
            <!--RuTitle-->
            <div class="mb-6">
            ...
            <div class="mb-6">
                ...
                <img src="{{ asset('storage/' .$certificate->image) }}" class="w-32">
                <div class="mb-6">
                    <div class="mb-1">
                    ...
                    <div>
                        <input name="image" id="image" type="file" value="{{ asset('storage/' . $certificate->image) }}">
                        @error('image')
                        <div class="text-red-500">{{ $message }}</div>
                        @enderror
                    </div>
